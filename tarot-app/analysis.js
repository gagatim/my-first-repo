...
// 每個主題準備 3 種不同切入角度的解讀（不是換字的同一句話），
// 分析時隨機挑一種，同一主題在不同次分析裡也不會長得一樣
const THEME_PRECISION = {
  begin: [
    (kw) => `這是一張關於「起點」的牌，重點不在於萬事俱備，而在於你願不願意先跨出那一步——「${kw}」正是這個起點目前最具體的樣貌。`,
    (kw) => `起點的能量正在你身邊發生，它可能還很小、還不成形，但「${kw}」已經是一個訊號：有些事情正準備要開始了，只是還沒被你正式承認。`,
    (kw) => `與其問「時機到了嗎」，這張牌更想問你「你準備好轉身了嗎」——「${kw}」是這個新起點目前唯一確定的線索。`,
  ],
  end: [
    (kw) => `這是一張關於「結束」的牌，它談的不是失去，而是騰出空間——「${kw}」點出了你現在最需要放下的是什麼。`,
    (kw) => `有些章節本來就該闔上，拖著不結束反而消耗更多——「${kw}」是這次牌陣提醒你該放手的地方。`,
    (kw) => `結束往往比開始更需要勇氣，因為它意味著承認某個版本的自己已經走到盡頭——「${kw}」正是那個需要被好好道別的部分。`,
  ],
  conflict: [
    (kw) => `這張牌指向一場正在發生的拉扯，無論對象是別人還是自己——「${kw}」是這場角力目前最明顯的樣子。`,
    (kw) => `衝突本身不是問題，逃避衝突才是——「${kw}」顯示這裡有一場你遲早得正面處理的角力，早點面對通常比拖延輕鬆。`,
    (kw) => `這張牌帶著劍拔弩張的氣味，值得問問自己：這場對抗，是在捍衛什麼重要的東西，還是只是不想認輸——「${kw}」是這股張力最直接的表現。`,
  ],
  clarity: [
    (kw) => `這是一張帶來清晰的牌，代表答案其實已經浮現，只是你可能還沒完全承認——「${kw}」是那道光照到的地方。`,
    (kw) => `真相通常不是新資訊，而是你早就知道、卻還沒說出口的事——「${kw}」正是那個你其實心裡有數的部分。`,
    (kw) => `這張牌像是一盞燈，照出了原本模糊的輪廓——「${kw}」是這份清晰目前最鮮明的一角，值得你多看一眼。`,
  ],
  confusion: [
    (kw) => `這張牌反映出一種尚未釐清的混亂，感受本身可能就是重點——「${kw}」是目前最需要被安放、而不是急著解決的部分。`,
    (kw) => `混亂的時候，急著找答案往往只會更亂——「${kw}」提醒你，這階段更需要的是允許自己先不知道。`,
    (kw) => `這張牌帶著一種說不清楚的不安，那種模糊感本身就是重要的資訊——「${kw}」是這團霧目前最靠近核心的地方。`,
  ],
  connection: [
    (kw) => `這是一張關於連結的牌，重點在於你和誰、或和自己的哪個部分正在靠近——「${kw}」是這份連結最核心的線索。`,
    (kw) => `這張牌談的是「靠近」這件事，可能是靠近另一個人，也可能是靠近你一直迴避的自己——「${kw}」是這段關係目前最真實的溫度。`,
    (kw) => `人與人之間的連結，往往藏在最不起眼的細節裡——「${kw}」正是這張牌想讓你留意的那個細節。`,
  ],
  isolation: [
    (kw) => `這張牌帶著獨自一人的況味，未必是壞事，有時候正是需要的距離——「${kw}」點出了你此刻與外界之間的那道界線。`,
    (kw) => `獨處不等於孤單，這張牌更像是在提醒你，有些答案只能在安靜下來後才聽得見——「${kw}」是這段距離目前的樣子。`,
    (kw) => `這張牌透露出一種「先靠自己」的姿態，不一定是被排除在外，也可能是你主動選擇的空間——「${kw}」是這道界線最明顯的地方。`,
  ],
  resource: [
    (kw) => `這是一張關於資源與現實的牌，談的是你手上實際擁有的籌碼——「${kw}」是目前最值得盤點的部分。`,
    (kw) => `比起想像中的困境，這張牌更想拉你回到現實層面——「${kw}」是你目前真正握在手裡、可以拿來用的東西。`,
    (kw) => `這張牌提醒你先別急著談理想，把手邊的資源盤點清楚——「${kw}」正是這份現實最具體的一角。`,
  ],
  instability: [
    (kw) => `這張牌透露出一種失衡的狀態，可能是資源、情緒或步調上的不對稱——「${kw}」是目前最容易被忽略、卻最該調整的地方。`,
    (kw) => `失衡不代表快要崩壞，通常只是在提醒你有個地方傾斜得比較久了——「${kw}」是那個該被重新校準的部分。`,
    (kw) => `這張牌像是一個還沒踩穩的姿勢，搖搖晃晃但還沒跌倒——「${kw}」點出了目前最需要找回平衡的地方。`,
  ],
  growth: [
    (kw) => `這是一張關於累積的牌，效果不會立刻顯現，但每一步都算數——「${kw}」正是這段累積過程目前最具體的樣子。`,
    (kw) => `這張牌沒有戲劇性的轉折，只有日復一日的累積，而那正是它的力量所在——「${kw}」是這份努力目前看得見的成果。`,
    (kw) => `慢慢來，是這張牌想告訴你的事——「${kw}」提醒你，現在還在打地基的階段，還不是驗收成果的時候。`,
  ],
  power: [
    (kw) => `這張牌關乎你能不能、願不願意掌握主導權——「${kw}」是你目前手上最實際的籌碼或姿態。`,
    (kw) => `主導權從來不是別人給的，是自己拿的——「${kw}」是這張牌想提醒你，此刻你其實比自己以為的更有選擇權。`,
    (kw) => `這張牌帶著一種「該你做決定了」的氣勢——「${kw}」是你目前最能施力、也最該負起責任的地方。`,
  ],
  surrender: [
    (kw) => `這是一張關於臣服與接受的牌，力量來自不再對抗——「${kw}」是你目前最需要練習放手信任的部分。`,
    (kw) => `有些戰場，贏的方法是不再打——這張牌談的正是這種力量，「${kw}」是你目前最值得練習鬆手的地方。`,
    (kw) => `療癒通常不是轟轟烈烈的事件，而是安靜地允許自己休息——「${kw}」是這份柔軟目前最需要被善待的角落。`,
  ],
};
...
const CATEGORY_TIEBACKS = [
  ...
  (cat) => `套進「${cat}」的情境，這句話幾乎是直接對你說的。`,
  (cat) => `如果先放下其他雜念，只看「${cat}」這件事，這裡的訊息會更清楚。`,
  (cat) => `對「${cat}」而言，這可能是整段分析裡最不該跳過的一句。`,
  (cat) => `這句話單獨拿出來看很抽象，但套進「${cat}」，具體到有點刺。`,
  (cat) => `別急著否認——這一句放在「${cat}」的脈絡裡，通常比你想的更準。`,
  (cat) => `從「${cat}」的角度重讀一次這句話，你可能會有不一樣的感覺。`,
  (cat) => `這裡的措辭乍看普通，但對照「${cat}」，其實相當直白。`,
  (cat) => `「${cat}」這件事，答案往往就藏在這種容易被忽略的細節裡。`,
  (cat) => `就算只看這一句，也值得你為了「${cat}」多停留幾秒。`,
  (cat) => `把這句話貼到「${cat}」上，你會發現它其實一點都不籠統。`,
];
...
const REFLECTION_PROMPTS = [
  ...
  "與其問「牌準不準」，不如問「這段話裡，哪一句我其實早就知道了」。",
  "闔上頁面前，給自己十秒鐘，讓剛剛讀到的東西沉澱一下，不急著解讀成好或壞。",
  "有時候塔羅牌的功能，不是告訴你答案，而是幫你把心裡已經有的答案，講得更清楚一點。",
  "如果今天只能帶走一句話離開，你會想帶走剛剛的哪一句？",
  "抽牌只是開始，接下來你打算怎麼做，才是真正決定結果的部分。",
  "把這次的牌陣記下來，過一陣子回頭看，通常會比現在更清楚哪句話說中了。",
  "有沒有哪張牌讓你下意識想反駁？那種抗拒感，往往才是最值得多想一下的地方。",
  "你不需要相信每一句話，只需要留意哪一句話讓你有反應。",
  "塔羅牌能做的，最多是幫你把問題問得更精準一點，答案還是你自己的。",
  "如果剛才有哪一段讓你笑了一下、或皺了一下眉，那大概就是重點。",
  "讀完之後別急著關掉頁面，讓自己多待一分鐘，看看情緒有沒有跟上。",
  "這不是一份判決書，比較像是一份邀請——邀請你多看自己一眼。",
  "有時候「準」的感覺，其實是因為你早就準備好要面對這件事了。",
  "比起「這代表什麼」，或許更值得問的是「這讓我想起了什麼」。",
  "抽牌前的那個念頭、那個問題，此刻回頭看，是不是也悄悄變了？",
  "別把這次的結果當成終點，它更像是這段時期裡的一張快照。",
  "有沒有哪張牌，你其實一眼就知道答案，只是還沒有勇氣承認？",
  "這副牌陣提供的是角度，不是命運，怎麼詮釋、怎麼行動，主導權一直都在你手上。",
  "如果現在心裡有點亂，那也沒關係，讓答案晚一點浮現，也是一種答案。",
  "把最打中你的那句話記下來，貼在看得到的地方，比記住整段分析更有用。",
  "有些牌義聽起來刺耳，但通常不是要否定你，而是想讓你看見自己一直迴避的部分。",
  "如果你正在猶豫一件事，這次的牌不會幫你做決定，但也許能幫你看清楚自己真正在猶豫什麼。",
  "讀牌最忌諱的，是只挑你想聽的部分相信——不妨也讀讀那句你想跳過的。",
  "這次的分析結束了，但真正的觀察，要從你走出這個頁面之後才開始。",
  "塔羅的價值不在預測未來，而在讓你誠實面對現在。",
  "有沒有哪張牌的位置，讓你覺得「怎麼剛好抽在這裡」？那種巧合感值得多想一下。",
  "別小看直覺——你選牌時那個瞬間的手感，往往已經知道答案了。",
  "如果這次的分析讓你有點不舒服，那可能代表它踩到了什麼真實的東西。",
  "答案不會馬上出現，但只要你願意繼續問下去，它遲早會浮上來。",
  "這段文字終究只是文字，真正重要的，是你讀完之後打算怎麼過接下來的日子。",
];
...
function generateAnalysis(spread, draws, question) {
  ...
  draws.forEach((draw, idx) => {
    ...
    const themeVariants = THEME_PRECISION[card.theme];
    let deepDive = themeVariants ? pickRandom(themeVariants)(primaryKeyword) : "";
    if (category) {
